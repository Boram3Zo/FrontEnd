...
- **API 테스트는 `scripts/` 폴더에 생성**: 실제 서버와의 통신 테스트용
- 파일명: `scripts/test-[기능명].mjs`
- 실제 HTTP 요청으로 백엔드 API 검증
- 개발 중 빠른 확인 및 디버깅 목적
- Node.js ESM 모듈 형식 사용 (`.mjs` 확장자)
...
async function test기능명() {
    try {
        // 실제 API 호출
        const response = await fetch("http://localhost:9988/api/endpoint", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
