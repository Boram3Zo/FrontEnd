...
export function RegionCourseList({
  ...
  useEffect(() => {
    ...
    loadRegionCourses();
  }, [selectedRegion, limit]); // currentLimit을 dependency에서 제거하여 무한 루프 방지
    ...
    <div className="px-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-sm text-gray-500">
          {courses.length}
          {allCourses.length > courses.length ? `/${allCourses.length}` : ""}개
          코스
        </span>
      </div>
